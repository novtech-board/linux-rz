...
static int rzsbc_mcu_bl_get_brightness(struct backlight_device *bd)
{
 ...
 int rzsbc_mcu_bl_update_status(struct backlight_device * bd)
 {
	...
	LOG_INFO("rzsbc_mcu_bl_update_status  brightness=%d power=%d fb_blank=%d state =%d  bd->props.brightness=%d\n", brightness, bd->props.power, bd->props.fb_blank, bd->props.state , bd->props.brightness);
...
{
	if (!connected) {
		printk("rzsbc_mcu_get_backlightdev is not ready\n");
		return NULL;
	}
	return bl;
}
...
static ssize_t rzsbc_mcu_bl_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int value;
